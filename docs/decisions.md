...
- **범위**: 서울 광진구 급속 충전소 46개소 · 충전기 116기 (이용자 제한 충전기 제외)
...
| Docker Compose 서비스 구성 | ✅ `nginx`, `postgres`, `redis`, `api`, `collector`, `ml` 6개 — collector는 API와 같은 이미지를 쓰되 별도 컨테이너로 분리해 API 재배포 중에도 수집이 끊기지 않게 함. ml은 예측 서빙 전용 컨테이너로 분리(collector만 호출, Redis 쓰기도 collector만). 외부 공개는 nginx만, 나머지는 Compose 내부 네트워크로만 접근. 6개 컨테이너 + 모델 상주가 2GB에 들어가는지는 `AI-04`에서 확인 | 장승민 |
| EC2 인스턴스 | ✅ t3.small (2GB RAM) | 장승민 |
| 서버 OS | ✅ Ubuntu 22.04 LTS | 장승민 |
| DB 백업 | ✅ EC2 내 cron으로 `pg_dump` 매일 1회 실행, 로컬 디스크에 최근 7일치 보관 | 장승민 |
...
| 수집 호출 설계 | ✅ 5분마다 `getChargerInfo`를 `zscode=11215`(광진구), `numOfRows=2000`으로 1회 호출(광진구 전체 1,492건이 한 페이지에 다 들어옴) → 우리 116개 충전기만 필터링해 저장. 하루 1콜×288사이클 = **288건**, 한도 1,000건 대비 여유 있음 | 장승민 |
...
| 시간 표기 (`BE-09`) | ✅ DB는 `timestamptz`로 UTC 저장, API 응답은 `+09:00` 오프셋 포함 ISO 8601, 학습 · 서빙 피처(시간대 · 요일)는 KST로 변환해 생성. API 원본 날짜는 출처 시간대 확인 후 UTC로 변환해 저장 | 장승민, 전원 |
