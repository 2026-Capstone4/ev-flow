...
def fetch_all_chargers(zscode: str) -> list[dict]:
    ...
    num_of_rows = 100
    while True:
        items = get_charger_info(page_no=page_no, num_of_rows=num_of_rows, zscode=zscode)
        if not items:
            break
...
def main():
    ...
    print(f"광진구 전체 조회: {len(all_items)}건")
    ...
    output_path = Path(__file__).resolve().parent.parent / "data" / "gwangjin_fast_chargers.csv"
